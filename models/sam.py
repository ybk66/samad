...
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None):
        ...
        #如果编码器模式“evp”，则冻结除提示嵌入、掩码解码器和提示编码器外的所有参数
    ...
    def set_input(self, input, gt_mask):#将图片和对应的GT放到指定设备
    ...
    def postprocess_masks(#掩码后处理
        ...
        """
        masks = F.interpolate(#双线性插值
            masks,
            (self.image_encoder.img_size, self.image_encoder.img_size),
            mode="bilinear",
            align_corners=False,
        )
        masks = masks[..., : input_size, : input_size]# 保留了 masks 张量的最后两个维度中的前 input_size 个元素。
